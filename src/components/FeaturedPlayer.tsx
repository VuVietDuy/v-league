...
const FeaturedPlayer = (props: any) => {
  const { player } = props;
  return (
    <Link to={`/players/${player.id}`} className="border rounded">
      <div className="flex relative ">
        <div className="w-[100px] h-[320px] p-3">
          <img className="w-full mb-2" src={player.club.logoURL} alt="" />
          <p className="text-sm">Đã chơi</p>
          <p className="font-bold text-xl mb-2">{player.appearances}</p>
          <p className="text-sm">Ghi bàn</p>
          <p className="font-bold text-xl mb-2">{player.goals}</p>
          <p className="text-sm">Hỗ trọ</p>
          <p className="font-bold text-xl mb-2">{player.assists}</p>
          <p className="text-sm">Shots</p>
          <p className="font-bold text-xl mb-2">{player.shots}</p>
        </div>
        <div className="flex-1 w-[200px] h-[320px]">
          <img
            src={player.imageURL}
